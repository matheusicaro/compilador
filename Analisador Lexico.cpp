...
//____________________________________________________________________________________________FUNÇÕES: TABELA DE SINAIS_______________________________________________________________
...
//maquina para analise lexica de uma CONSTANTE				
				case 4:
					 ...
					 if (isdigit(entrada[i]) || entrada[i] == '+' || entrada[i] == '-'){
						if(isdigit(entrada[i+ 1]) || isdigit(entrada[i])){				//condição de que -55 é uma constante, 45-55 é uma constante(45), addop(-), constante(55)
							lexeme[j] = entrada[i];
							estado = 5;
							break;
						}
...
//maquina para analise lexica de um MULOP
				case 13:
					...
					if (entrada[i] == '*' || entrada[i] == '/'){
						...
						lexeme[j] = entrada[i];
						i++;
						cout<<"\n\nERRO LEXICO - REVISE O LEXEME.....: "<<lexeme[j]<<"\n";
						estado = 1;  
						j = 0;
						break;
					}
